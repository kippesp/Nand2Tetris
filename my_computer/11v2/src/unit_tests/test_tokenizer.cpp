...
SCENARIO("Tokenizer baseline checks")
{
  SECTION("tokenizer")
  {
    ...
    REQUIRE(tokens[0].get().type == TokenType_t::T_SYMBOL);
    REQUIRE(tokens[0].get().value_enum == TokenValue_t::J_PLUS);

    REQUIRE(tokens[1].get().type == TokenType_t::T_INTERNAL);
...
SCENARIO("Tokenize Jack symbols")
{
  ...
  SECTION("tokenizer")
  {
    ...
    {
      REQUIRE(tokens[i].get().type == TokenType_t::T_SYMBOL);
...
SCENARIO("Tokenize Jack keywords")
{
  ...
  SECTION("tokenizer")
  {
    ...
    for (decltype(tokens.size()) i = 0; i < tokens.size() - 1; i++)
    {
      REQUIRE(tokens[i].get().type == TokenType_t::T_KEYWORD);
      REQUIRE(tokens[i].get().value_enum == expected_parsed_tokens[i]);
    }
  }
}
...
SCENARIO("Tokenize integers")
{
  SECTION("single integers")
  {
    ...
    for (decltype(tokens.size()) i = 0; i < tokens.size() - 1; i++)
    {
      REQUIRE(tokens[i].get().type == TokenType_t::T_INTEGER_CONSTANT);
  ...
  SECTION("simple statement")
  {
    ...
    REQUIRE(tokens[0].get().type == TokenType_t::T_INTEGER_CONSTANT);
    REQUIRE(tokens[0].get().value_str == "10");

    REQUIRE(tokens[1].get().type == TokenType_t::T_SYMBOL);
    REQUIRE(tokens[1].get().value_enum == TokenValue_t::J_PLUS);

    REQUIRE(tokens[2].get().type == TokenType_t::T_INTEGER_CONSTANT);
    REQUIRE(tokens[2].get().value_str == "5");

    REQUIRE(tokens[3].get().type == TokenType_t::T_SYMBOL);
    REQUIRE(tokens[3].get().value_enum == TokenValue_t::J_ASTERISK);

    REQUIRE(tokens[4].get().type == TokenType_t::T_SYMBOL);
    REQUIRE(tokens[4].get().value_enum == TokenValue_t::J_MINUS);

    REQUIRE(tokens[5].get().type == TokenType_t::T_INTEGER_CONSTANT);
    REQUIRE(tokens[5].get().value_str == "2");

    REQUIRE(tokens[6].get().type == TokenType_t::T_INTERNAL);
...
SCENARIO("Tokenize identifiers")
{
  SECTION("multiple identifiers")
  {
    ...
    for (decltype(tokens.size()) i = 0; i < tokens.size() - 1; i++)
    {
      REQUIRE(tokens[i].get().type == TokenType_t::T_IDENTIFIER);
  ...
  SECTION("miscellaneous")
  {
    ...
    REQUIRE(tokens[0].get().type == TokenType_t::T_IDENTIFIER);
    REQUIRE(tokens[0].get().value_enum == TokenValue_t::J_IDENTIFIER);
    REQUIRE(tokens[0].get().value_str == "main");

    REQUIRE(tokens[1].get().type == TokenType_t::T_KEYWORD);
...
SCENARIO("Tokenize string")
{
  SECTION("simple strings")
  {
    ...
    for (decltype(tokens.size()) i = 0; i < tokens.size() - 1; i++)
    {
      REQUIRE(tokens[i].get().type == TokenType_t::T_STRING_CONSTANT);
...
SCENARIO("Tokenize comments")
{
  SECTION("line comments")
  {
    ...
    for (decltype(tokens.size()) i = 0; i < tokens.size() - 1; i++)
    {
      REQUIRE(tokens[i].get().type == TokenType_t::T_INTERNAL);
  ...
  SECTION("mixed comments")
  {
    ...
    REQUIRE(tokens[0].get().type == TokenType_t::T_KEYWORD);
    REQUIRE(tokens[0].get().value_enum == TokenValue_t::J_CLASS);

    REQUIRE(tokens[1].get().type == TokenType_t::T_SYMBOL);
    REQUIRE(tokens[1].get().value_enum == TokenValue_t::J_PLUS);

    REQUIRE(tokens[2].get().type == TokenType_t::T_INTERNAL);
...
SCENARIO("Tokenizer lexical issues")
{
  SECTION("unterminated block commment")
  {
    ...
    REQUIRE(tokens[0].get().type == TokenType_t::T_UNDEFINED);
  ...
  SECTION("empty file")
  {
    ...
    REQUIRE(tokens[0].get().type == TokenType_t::T_INTERNAL);
